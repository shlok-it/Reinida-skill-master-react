...
const EditDistrict = (props) => {
    ...
    const submitThis = (e) => {
        ...
        if (values.state_code == '') {
            ...
            setLoading(true);
            call_secure_api('location/district/update', values)
                .then(
                    (resolve) => {
    ...
    return (
        <Modal size={"lg"} fade={false} isOpen={true} backdrop="static" keyboard={false}>
            <ModalHeader close={<button type={'button'} className="btn-close" onClick={() => { props.model_handler(false) }}></button>}><CardText>Update District</CardText></ModalHeader>
            <ModalBody>
