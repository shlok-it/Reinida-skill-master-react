...
const Index = () => {
    ...
    useEffect(() => {
        get_emp_list();
        dispatch(changeBCSubTitle('Block List'));
    }, []);
    const get_emp_list = (
        pagenum = page,
		size = perPage,
        ...
        call_secure_get_api(
        `location/block?page=${pagenum}&per_page=${size}&colName=${col}&sortBy=${sort}&search_key=${search_key}&delay=1`)
            .then(
                (resolve) => {
    ...
    const addBlock = () => {
        setDisplay(<AddBlock model_handler={parent_handler} />);
    }
    const editBlock = (item) => {
        setDisplay(<EditBlock item={item} model_handler={parent_handler} />);
    }
                ...
                cell: (d) => {
                    return (
                        <div className='d-flex'>
                            <button className="btn btn-xs btn-success text-nowrap" onClick={(e) => { editBlock(d) }}><i className='bx bxs-edit'></i> Edit</button>                                                       
                        </div>
                    );
                },
                wrap: true,
            ...
            <div className="card radius-10">
                ...
                <div className="d-lg-flex align-items-center mb-4 gap-3">
                        ...
                        <div className="ms-auto">
                            <button onClick={() => addBlock()} className="btn btn-primary radius-30 mt-2 mt-lg-0"><i className="bx bxs-plus-square"></i>Add Block</button>
                        </div>
					</div>
