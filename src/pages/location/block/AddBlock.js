...
const AddBlock = (props) => {
    ...
    const [values, setValues] = useState({
        name: '', state_code: '', status: 1, district_id: ''
    });
    useEffect(() => {
    ...
    const submitThis = (e) => {
        ...
        if (values.state_code == '') {
            ...
            setLoading(true);
            call_secure_api('location/block/add', values)
                .then(
                    (resolve) => {
    ...
    return (
        <Modal size={"lg"} fade={false} isOpen={true} backdrop="static" keyboard={false}>
            <ModalHeader close={<button type={'button'} className="btn-close" onClick={() => { props.model_handler(false) }}></button>}><CardText>Add Block</CardText></ModalHeader>
            <ModalBody>
