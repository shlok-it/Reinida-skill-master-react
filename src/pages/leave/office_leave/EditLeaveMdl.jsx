...
const EditLeaveMdl = (props) => {
    const [loading, setLoading] = useState(false);
    const [values, setValues] = useState(props.leave);
    ...
    const submit_office_leave = (e) => {
        ...
        if (values.leave_resion == '') {
            ...
            setLoading(true);
            call_secure_api('leave/office/update', values)
                .then(
                    (resolve) => {
    ...
    return (
        <Modal size={"md"} fade={false} isOpen={true} backdrop="static" keyboard={false}>
            <ModalHeader close={<button type={'button'} className="btn-close" onClick={() => { props.model_handler(false) }}></button>}><CardText>Edit Leave</CardText></ModalHeader>
            <ModalBody>
                <form action="" onSubmit={submit_office_leave}>
                    <div className="form-group row">
                        ...
                        <div className="col-sm-12">
                            ...
                            <input className="form-control form-control-sm" type="date" min={today_date}  value={values.from_date} name="from_date" onChange={set('from_date')} />
                        </div>
