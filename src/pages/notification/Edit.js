...
const Edit = (props) => {
    ...
    const { emp_list } = useStateContext();
    useEffect(() => {
        if(props.data.user_id>0){
            set_target_users(parseInt(props.data.user_id));
        }
        // console.log(values,'values')
        // setTimeout(()=>{
        //     console.log(props.data.user_id,'user id');
        //     console.log(values);
        // },3000)
    }, []);
    ...
    const submitThis = (e) => {
        ...
        if (values.title == '') {
            ...
            setLoading(true);
            call_secure_api('notification/update', values)
                .then(
                    (resolve) => {
    ...
    return (
        <Modal size={"lg"} fade={false} isOpen={true} backdrop="static" keyboard={false}>
            <ModalHeader close={<button type={'button'} className="btn-close" onClick={() => { props.model_handler(false) }}></button>}><CardText>Edit Notification</CardText></ModalHeader>
            <ModalBody>
                <form action="" onSubmit={submitThis}>
                    <div className="row g-3">
                        ...
                        <div className="col-lg-12 col-xs-12 col-sm-12 col-md-12">
                            <div className="form-group">
                                <label htmlFor="notification_text" className="control-label"><span className="text-danger">*</span>Notification Text</label>
