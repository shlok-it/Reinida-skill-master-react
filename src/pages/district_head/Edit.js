...
const Edit = (props) => {
    ...
    const [values, setValues] = useState({
        ...
        state_code:props.item.state_code,
        district_id:props.item.district_id,
        status:props.item.status,
        session_in:props.item.session_in,
        session_out:props.item.session_out
    });
    useEffect(() => {
        setValues(oldValues => ({ ...oldValues, district_id: props.item.district_id }));
    }, [values.state_code])
    ...
    const submitThis = (e) => {
        ...
        if (values.name == '') {
            ...
            setLoading(true);
            call_secure_api('district_head/update', values)
                .then(
                    (resolve) => {
