...
const EditList = (props) => {
  ...
  return (
    ...
    >
      ...
      >
        <CardText><strong className={"check_header check_bg_"+ props.employee.phototype }>Check {props.employee.phototype} <span>{props.employee.created_at}</span></strong></CardText>
      </ModalHeader>
      <ModalBody>
          <div className="row">
          ...
          <div className="text-center py-3">
            <button
              onClick={(e) => {
                props.deleteEmp(values.id);
              }}
              disabled={loading}
              className="border-0 btn btn-danger btn-gradient-primary btn-rounded"
            >
              {loading && <i className="mdi mdi-dots-circle mdi-spin"></i>} Delete
            </button>
            &nbsp;&nbsp;
            <button
              type="button"
