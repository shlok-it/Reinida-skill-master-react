...
const Slider = () => {
  ...
  useEffect(() => {
    get_emp_list();
    dispatch(changeBCSubTitle("Slider List"));
  }, []);

  const get_emp_list = () => {
    call_secure_get_api("slider/getImge").then(
      (resolve) => {
  ...
  const updateSequence = () => {
    call_secure_api("slider/updateImageSequence", { userData: tbNewsIndex }).then(
      (resolve) => {
        if (resolve.status === true) {
  ...
  const updateStatus = (id) => {
    call_secure_api("slider/updateImageStatus", { id:id }).then(
      (resolve) => {
        if (resolve.status === true) {
          toast.success(resolve.message, "success", 5000);
          get_emp_list();
  ...
  const delete_emp = (id) => {
    call_secure_api("slider/deleteImage", { emp_id: id }).then(
      (resolve) => {
        if (resolve.status === true) {
          toast.success(resolve.message, "success", 5000);
          get_emp_list();
