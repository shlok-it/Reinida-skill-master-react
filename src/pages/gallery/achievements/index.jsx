...
const Slider = () => {
  ...
  useEffect(() => {
    get_emp_list();
    dispatch(changeBCSubTitle("Achievements List"));
  }, []);

  const get_emp_list = () => {
    call_secure_get_api("achievements/getImge").then(
      (resolve) => {
  ...
  const updateSequence = () => {
    call_secure_api("achievements/updateImageSequence", { userData: tbNewsIndex }).then(
      (resolve) => {
        if (resolve.status === true) {
  ...
  const updateStatus = (id) => {
    call_secure_api("achievements/updateImageStatus", { id:id }).then(
      (resolve) => {
        if (resolve.status === true) {
          toast.success(resolve.message, "success", 5000);
          get_emp_list();
  ...
  const delete_emp = (id) => {
    call_secure_api("achievements/deleteImage", { emp_id: id }).then(
      (resolve) => {
        if (resolve.status === true) {
          toast.success(resolve.message, "success", 5000);
          get_emp_list();
          ...
          <div className="d-lg-flex align-items-center mb-4 gap-3">
            ...
            {currentUser.master_role != "MANAGER" && (
              <div className="ms-auto">
                ...
                >
                  <i className="bx bxs-plus-square"></i>Add Image
                </button>
              </div>
            )}
          </div>
                      ...
                      >
                        {(provider) => (
                          ...
                          >
                            ...
                            <td>{index + 1}</td>
                            <td>
                              <div className="img-achievmente-cat">
                                <img
                                  src={
                                    item.image_link
